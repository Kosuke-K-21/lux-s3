...
class Agent:
    def __init__(self, player: str, env_cfg: dict) -> None:
        ...
        self.opp_player: str = "player_1" if self.player == "player_0" else "player_0"
        self.team_id: int = 0 if self.player == "player_0" else 1
        self.opp_team_id: int = 1 if self.team_id == 0 else 0
        np.random.seed(0)
        self.env_cfg = env_cfg

        self.relic_node_positions: list = []
        self.discovered_relic_nodes_ids: set = set()
        self.unit_explore_locations: dict = dict()
